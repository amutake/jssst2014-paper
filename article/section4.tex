...
Coq では簡単にするためにその情報を捨て，\srcref{coq:tmp} のように任意の {\tt name} 型の値から任意の {\tt option star} 型への関数とした．定義域の範囲に入っていない入力は {\tt None} を返すことを想定している，
...
また，以上の5つについて仮名関数の性質を満たすことを証明した．
...
%% (~だから ACT は4つに分割できる．ch はここでしかでてこないから ch は3パターンに分類できる)

%% なぜ3パターンしかないことがわかったのかは後述? (型付け規則のとこで，ch は ACT ルールでしか出てこなくて，それも4つにパターン分けできる．そのうち1つは要素が0個，2は要素が1個，残りの1つは要素が2個．)

%% \api の型付け規則を素直に定義する前に，具体化できるものはできるだけ具体化しておく．



%% ACT ルールの簡単化のため，$ \rho $ の要素数によって場合分けをする．
...
%% 以上から \textsc{Act} 規則は4つに分割することができる．
ここで，$ch$ 関数の入力としてとりうる組は要素数が0，1，2のものしかないことに注意されたい．
...
型付け規則 \textsc{Case} は，前提の数がパターンに依存して変動するので，Coq で定義しにくい．よって \srcref{api:case_split} のように \textsc{Case} をパターンが $0$ 個の場合と $i + 1$ に分割し，帰納的に定義する．これで，\textsc{Case} 規則の前提の数はパターンの数によらず一定となる．
...
\begin{figure*}[t]
  \infrule[Res]{
    ...
    \typing{\rho - \{x\}}{f|x}{\restrictconf{x}{P}}
  }
...
Coq による定義では振る舞いの雛形からアクターを作る\conf を二つに分割している．よって型付け規則も二つに分割する必要がある．\srcref{api:inst_rule} のようになる．
...
この公理は Coq の標準ライブラリで提供されているものを使った．%%  \srcref{coq:fun_ext} のように定義されている．
...
%%       $\typing{\rho_1 \cup \rho_2}{f_1 \oplus f_2}{P_1 | P_2}$ となる型付け規則は \textsc{Comp} のみであり，かつ\recep は $\rho_1, \rho_2$ ，\tmp は $f_1,f_2$ で構築されている．よって命題を満たす．
%%     \item[(\textsc{Res})]
