...
本章では，アクターモデルの振る舞いを強制するために $ \pi $ 計算に型を付けた \api について説明する．
まず準備として記号の定義をし．\api の\conf の定義をする．次に型システムを導入するが，その際に仮名関数という関数を導入し，型付け規則を定義する．最後にこの型システムにおける健全性について述べる．
...
\conf の文法は，\srcref{api_config} のようになっている．
...
$ f^{*}(f(x)) = \bot $ は，仮の名前のアクターはさらに仮の名前を持つことはできないという性質である．
...
\begin{dfn}[\api の健全性]
  \conf $P$ について，$\typing{\rho}{f}{P}$ で型付けできたとき．以下の3つの補題を満たすならば，この型システムは健全である．

  \begin{lem}[\recep の健全性]
    $\rho$ は $P$ 中に現れる\free の部分集合
  \end{lem}

  \begin{lem}[仮名の取り方の健全性]
    以下の3つを満たす．
    \begin{enumerate}
      \item $ f(x) \neq x $
      \item $ f^{*}(f(x)) = \bot $
