...
ただし，$B$ は振る舞いの雛形で，$B \defeq \behaviortemplate{\tilde{x}}{\tilde{y}}{x_1}{z}{P}$ である．
これは，$\tilde{x}$ という名前の組と，$\tilde{y}$ という名前の組を受け取り，$\tilde{x}$ の第一要素 $x_1$ を名前とし，メッセージ $v$ を受け取って $P$ を実行するようなアクターを返す関数であることを意味する．
また，$\tilde{x}$ の要素数は1または2とすし，この理由は \ref{typing_rule} 節で示す．
